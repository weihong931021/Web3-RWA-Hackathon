 ...
 * 添加用戶到白名單
 */
export async function addToWhitelist(userAddress: string): Promise<ethers.TransactionReceipt> {
  const rwaContract = await getRWAContract()
...
export async function lockAndBorrow(
  ...
  console.log('🔒 開始借貸流程')
  console.log('  原始 RWA 金額:', rwaAmount, '| Type:', typeof rwaAmount, '| Value:', JSON.stringify(rwaAmount))
  console.log('  原始 USDC 金額:', usdcAmount, '| Type:', typeof usdcAmount, '| Value:', JSON.stringify(usdcAmount))
  ...
  console.log('  parseUnits(', rwaAmountStr, ', 18) - Type:', typeof rwaAmountStr)
  console.log('  parseUnits(', usdcAmountStr, ', 6) - Type:', typeof usdcAmountStr)
 ...
 */
export async function repayLoan(repayAmount: string): Promise<ethers.TransactionReceipt> {
  const usdcContract = await getUSDCContract()
  const lendingPool = await getLendingPoolContract()
 ...
 */
export async function unlockCollateral(rwaAmount: string): Promise<ethers.TransactionReceipt> {
  const lendingPool = await getLendingPoolContract()
 ...
 * 清算貸款
 */
export async function liquidateLoan(userAddress: string): Promise<ethers.TransactionReceipt> {
  const usdcContract = await getUSDCContract()
  const lendingPool = await getLendingPoolContract()
 ...
 * 為借貸池注入流動性
 */
export async function fundPool(usdcAmount: string): Promise<ethers.TransactionReceipt> {
  const usdcContract = await getUSDCContract()
  const lendingPool = await getLendingPoolContract()
 ...
 * 計算健康度
 */
export async function calculateHealthRatio(userAddress: string): Promise<number> {
  const loan = await getUserLoan(userAddress)
...
export async function getProtocolInfo(): Promise<{
  ...
  const [ltv, liquidationThreshold, priceOracleAddr, rwaTokenAddr, usdcTokenAddr] =
    await Promise.all([
      lendingPool.LTV(),
 ...
 * 檢查用戶合規性
 */
export async function checkUserCompliance(userAddress: string): Promise<boolean> {
  const lendingPool = await getLendingPoolContract()
...
export async function getPoolBalance(): Promise<string> {
  const usdcContract = await getUSDCContract()
  const balance = await usdcContract.balanceOf(CONTRACT_ADDRESSES.FluidPay_LendingPool)
 ...
 * 鑄造測試代幣（僅用於測試）
 */
export async function mintUSDC(toAddress: string, amount: string): Promise<ethers.TransactionReceipt> {
  const usdcContract = await getUSDCContract()
...
export async function mintRWA(toAddress: string, amount: string): Promise<ethers.TransactionReceipt> {
  const rwaContract = await getRWAContract()
